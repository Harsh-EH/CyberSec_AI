...
cti_path = r"C:\Users\harsh\OneDrive\Desktop\CyberSec_AI\backend\CyberSec_AI\cti\enterprise-attack\enterprise-attack.json"
try:
...
def map_threat_to_mitre(threat_label, packet_count, packets):
    """
    Map detected threats to MITRE ATT&CK techniques and tactics based on threat label and packet behavior.
    Args:
    ...
    Returns:
        String containing MITRE ATT&CK technique IDs, names, and tactics
    """
    if not mitre_data:
    ...
    # Extract common protocols, ports, and packet size statistics
    protocols = set()
    ports = set()
    ...
    # Enhanced heuristic mapping based on threat label and packet behavior
    if threat_label == "Malware":
        ...
        if (80 in ports or 443 in ports) and ("TCP" in protocols):
            ...
            if technique:
                # Look up the associated tactic
                tactic_ids = technique.get('kill_chain_phases', [])
                tactic_name = "Unknown Tactic"
                for phase in tactic_ids:
                    if phase['kill_chain_name'] == 'mitre-attack':
                        tactic = mitre_data.get(f"x-mitre-tactic--{phase['phase_name']}")
                        if tactic:
                            tactic_name = tactic['name']
                        else:
                            # Fallback: Map phase_name to a known tactic name
                            tactic_name = phase['phase_name'].replace('-', ' ').title()
                        break
                mitre_mapping.append(f"{technique['external_references'][0]['external_id']}: {technique['name']} (Tactic: {tactic_name})")
        # Check for command and control communication (high packet rate, small packets)
        if packet_rate > 10 and avg_packet_size < 200:
            ...
            if technique:
                ...
                for phase in tactic_ids:
                    if phase['kill_chain_name'] == 'mitre-attack':
                        ...
                        break
                mitre_mapping.append(f"{technique['external_references'][0]['external_id']}: {technique['name']} (Tactic: {tactic_name})")
        # Check for potential brute force (high packet rate to specific ports like 22 or 3389)
        if (22 in ports or 3389 in ports) and packet_rate > 20:
            technique = mitre_data.get("attack-pattern--a93494bb-4b80-4ea1-8695-59c0d9a8b072")  # T1110: Brute Force
            if technique:
                tactic_ids = technique.get('kill_chain_phases', [])
                tactic_name = "Unknown Tactic"
                for phase in tactic_ids:
            ...
            if technique:
                ...
                for phase in tactic_ids:
                    if phase['kill_chain_name'] == 'mitre-attack':
                        ...
                        break
                mitre_mapping.append(f"{technique['external_references'][0]['external_id']}: {technique['name']} (Tactic: {tactic_name})")
        # Check for SQL injection attempts (port 1433, specific packet patterns)
        if 1433 in ports and any(b"SELECT" in bytes(packet).upper() for packet in packets):
            technique = mitre_data.get("attack-pattern--0f20e3cb-245b-4a4e-8f7d-d0db0024833b")  # T1190: SQL Injection
            if technique:
                tactic_ids = technique.get('kill_chain_phases', [])
                tactic_name = "Unknown Tactic"
                for phase in tactic_ids:
